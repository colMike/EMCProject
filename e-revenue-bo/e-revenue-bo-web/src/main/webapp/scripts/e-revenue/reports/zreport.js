 ...
 */
'use strict';
angular.module('app.zreport', []).controller("zReportCtrl", ["$scope", "$filter", "zReportSvc","memStatementSvc", "$zReportValid", "$rootScope", "blockUI", "logger", "$location","$http","$window", "wardSvc",function ($scope, $filter, zReportSvc,memStatementSvc, $zReportValid, $rootScope, blockUI, logger, $location,$http,$window,wardSvc) {
    $scope.mem=[];
