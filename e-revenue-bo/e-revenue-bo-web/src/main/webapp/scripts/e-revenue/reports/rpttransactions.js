 ...
 */
'use strict';
angular.module('app.rptTransaction', []).controller("rptTransactionCtrl", ["$scope", "$filter", "memStatementSvc", "$memStatementValid", "$rootScope", "blockUI", "logger", "$location","$http","$window","$timeout","wardSvc", function ($scope, $filter, memStatementSvc, $memStatementValid, $rootScope, blockUI, logger, $location,$http,$window,$timeout,wardSvc) {
    $scope.mem=[];
   ...
   $scope.previewReport=function(mem)
   {
       ...
       $scope.subcounty=mem.subCountySelect;
   ...
   $scope.loadTransactions=function(){
     ...
     gate.toDate=$scope.toDt;
       gate.subcounty=$scope.subcounty;
       console.log(".......... ",  gate.subcounty)
 ...
 //  tmrAuthData = $timeout($scope.onTimeout,1000);
   ...
   $scope.loadAmpuntDetail();
	$scope.exportPdfReport = function(mem) {
		...
		if (!$memStatementValid(mem.FromDt)) {
            ...
            return false;
        }else if (!$memStatementValid(mem.subCountySelect)) {
        ...
        console.log(subcounty);
			...
			$scope.url = '/erevenue/reports?type=TX&eType=P&FrDt='+ FromDt + '&ToDt=' + ToDt +'&subcounty='+subcounty;
   ...
   $scope.exportExcelReport = function(mem) {
       ...
       console.log(subcounty);
			...
			$scope.url = '/erevenue/reports?type=TX&eType=E&FrDt='+ FromDt + '&ToDt=' + ToDt +'&subcounty='+subcounty;
	}
